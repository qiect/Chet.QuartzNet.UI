...
![分析页](docs/images/analytices.png)

![作业管理](docs/images/job.png)

![Cron帮助](docs/images/cron-helper.png)

![日志管理](docs/images/log.png)

![日志详情](docs/images/log-detail.png)

![通知管理](docs/images/notification.png)

![通知详情](docs/images/notification-detail.png)

![微信通知](docs/images/wechat-notification-pushplus.png)
...
#### 新增
- 数据迁移可视化功能：支持文件存储到数据库存储的可视化迁移，采用卡片式 UI + 时间线 + 进度条交互，实时展示迁移进度与状态

#### 优化
- 分析页查询改为数据库侧聚合（GroupBy）并禁用 EF Core 变更跟踪（AsNoTracking），消除全量加载日志导致的性能问题
- 修复健康象限图大气泡覆盖小气泡无法选中的问题，优化象限图 UI 交互

#### 修复
- 修复执行活跃度/质量/周同比数据基于小时而非天聚合的问题，确保统计维度正确
- 修复健康象限图大气泡覆盖小气泡无法选中的交互问题

#### 兼容性

- 与现有 .NET 8.0 / 10.0 版本完全兼容
- 数据库存储模式需同步更新 EFCore 扩展包

### [2.5.0] - 2026-8-30

#### 新增
- 重构分析页，包括 KPI 卡片，新增作业健康象限图表、时段分布热力图、耗时 Top 排名
- 作业管理表格支持作业启用/禁用开关，Switch 交互切换，操作前弹出确认对话框
- 作业对话框功能调整，新增 API 请求头 Key-Value 对编辑器等
- 新增作业禁止并发执行功能，实体/DTO/API 新增 DisallowConcurrentExecution 字段，Quartz JobBuilder 集成，4个数据库提供程序 EF Core 迁移（注意：数据库存储模式需同步更新包）

#### 优化
- 优化作业编辑表单布局与交互体验
- 统一所有对话框宽度为 800px
- 日志/通知列表"查看详情"图标更换为 info-circle-fill

#### 修复
- 修复中英文切换后表格列头、搜索表单标签不更新的问题
- 修复日志详情 Descriptions 执行时长列过窄的问题
- 修复系统设置表单英文 label 换行的问题

#### 兼容性

- 与现有 .NET 8.0 / 10.0 版本完全兼容
- 数据库存储模式需同步更新 EFCore 扩展包（新增 DisallowConcurrentExecution 字段迁移）

### [2.4.0] - 2026-8-20

#### 新增
- 新增失败重试配置组，支持失败重试次数、重试间隔时间等配置（注意：数据库存储模式需同步更新包）
- 新增Cron表达式生成器，支持预览近10次执行时间
...
#### 新增
- 系统配置管理：新增 system-config 页面，配置服务名/环境/描述，用于多实例区分
- vxe-table 全面接入：job/log/notification 三页迁移到 useVbenVxeGrid，支持服务端分页排序
- 日志多条件查询：支持开始/结束时间范围筛选（ISO 8601 格式）
- 分析页重写：图表重构 + 顶部服务标识融合 + KPI 卡片图标优化

#### 优化
- 详情对话框：状态色条 + Descriptions 元数据 + 简化 header
- 作业编辑页：分区布局（110px label，长短字段分列）
- 所有 Modal 支持拖拽，vxe-table 排序状态持久化
- job 操作列改为 hover 触发，时长列改为秒显示
...
#### 优化
- 新增编辑作业时，API超时时长调整为1-99999秒
- 调整页面默认大小为20
- 添加页面缓存，切换页面时保持当前状态

#### 修复
- 修复新增编辑作业时输入验证失败导至的异常问题
- 修复日志切换分页时排序异常问题
...
- 重构分析页
- 优化详情对话框、通知配置对话框、Cron 表达式帮助对话框 UI

#### 修复

- 修复认证过期后多次提示
- 修复 .Net 8 框架 下 SQLite 版本冲突问题
